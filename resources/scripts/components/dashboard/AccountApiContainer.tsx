...
export default () => {
    ...
    return (
        <PageContentBlock title={'API de conta'}>
            ...
            <div css={tw`md:flex flex-nowrap my-10`}>
                ...
                <ContentBox title={'API Keys'} css={tw`flex-1 overflow-hidden mt-8 md:mt-0 md:ml-8`}>
                    ...
                    >
                        Todas as solicitações usando o <Code>{deleteIdentifier}</Code> chave será invalidada.
                    </Dialog.Confirm>
                    {keys.length === 0 ? (
                        <p css={tw`text-center text-sm`}>
                            {loading ? 'Carregando...' : 'Não existem chaves de API para esta conta.'}
                        </p>
                    ) : (
                        keys.map((key, index) => (
                            <GreyRowBox
